  ...
  <header class="w-full text-text-opposite bg-primary">
    <nav class="container flex items-center justify-between p-6 lg:px-8" aria-label="Global">
      ...
      <div class="hidden lg:flex lg:items-center gap-4">
        ...
        <button class="btn bg-secondary text-text-primary font-bold">
          {{ $t('main.login') }}
        </button>
      </div>
    </nav>
    ...
    <Dialog as="div" class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
      ...
      <DialogPanel
        class="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-primary-800 text-text-opposite px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10"
      >
        ...
        <div class="mt-6 flow-root">
          <div class="-my-6 divide-y divide-gray-500/10">
            <div class="py-6">
              <button class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7">
                {{ $t('main.login') }}
              </button>
            </div>
          </div>
